...
`any`, `!`, `as`, `@ts-expect-error`, and `eslint-disable` are **escape hatches**: each one in this repo carries a one-line comment naming what makes it unavoidable. An uncommented one is a defect in review.
